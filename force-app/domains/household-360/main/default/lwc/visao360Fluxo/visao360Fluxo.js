...
export default class Visao360Fluxo extends LightningElement {
    @api fluxo = null;
    ...
    get linhas() {
        const max = Number(this.fluxo?.maximo ?? 10000);
    ...
    get barras() {
        ...
        return meses.map((m, i) => {
            ...
            const altEntrada = Math.max(2, (entrada / max) * ALTURA_PLOT);
            const altSaida = Math.max(2, (saida / max) * ALTURA_PLOT);
            return {
                ...m,
                idEntrada: `${m.mes}-e`,
                idSaida: `${m.mes}-s`,
