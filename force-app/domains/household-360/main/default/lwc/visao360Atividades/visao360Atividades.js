...
export default class Visao360Atividades extends LightningElement {
    @api interacoes = [];
    ...
    get interacoesVisiveis() {
        return this.interacoesFiltradas.slice(0, this.limite).map((i) => ({
            ...
            expandido: this.expandidos.includes(i.id),
            rotuloVerMais: this.expandidos.includes(i.id) ? 'Ver menos' : 'Ver mais'
        }));
    }
    ...
    aoAlternarDetalhe(event) {
        const id = event.currentTarget?.dataset?.id;
        if (!id) return;
        this.expandidos = this.expandidos.includes(id)
            ? this.expandidos.filter((e) => e !== id)
            : [...this.expandidos, id];
    }
